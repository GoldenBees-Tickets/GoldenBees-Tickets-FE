      ...
      <div className="flex flex-col lg:flex-row space-x-6 mb-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-700 mb-2">
            ...
            <Link to={`/director/${ListMovie?.Director?.id}`} className="text-blue-600 hover:underline">{directorName}</Link>
          </p>
          <p className="text-sm text-gray-700 mb-2">
            <strong>Diễn viên:</strong>
            <span>
              {ListMovie?.MovieActors?.map((actorRelation, index) => (
                <Link key={actorRelation.Actor?.id} to={`/actor/${actorRelation.Actor?.id}`} className="text-blue-600 hover:underline">
                  {actorRelation.Actor?.name}{index < ListMovie.MovieActors.length - 1 ? ", " : ""}
                </Link>
              ))}
            </span>
          </p>
          <p className="text-sm text-gray-700 mb-2">
            ...
            <span>
              {ListMovie?.MovieProducers?.map((producerRelation, index) => (
                <Link key={producerRelation.Producer?.id} to={`/producer/${producerRelation.Producer?.id}`} className="text-blue-600 hover:underline">
                  {producerRelation.Producer?.name}{index < ListMovie.MovieProducers.length - 1 ? ", " : ""}
                </Link>
              ))}
            </span>
          </p>
          <p className="text-sm text-gray-700 mb-4"><strong>Thời gian:</strong> {ListMovie?.duration} phút</p>
          <p className="text-sm text-gray-700 mb-4"><strong>Độ tuổi:</strong> {ListMovie?.age_rating == 0 ? "Mọi lứa tuổi" : `Trên ${ListMovie?.age_rating} được xem`}</p>

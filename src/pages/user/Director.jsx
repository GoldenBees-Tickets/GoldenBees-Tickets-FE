...
export default function Director() {
  const { id } = useParams(); 
  const { data: list, error, isLoading } = useGetDirectorsQuery(); 

  const director = list?.directors?.find((director) => director.id === parseInt(id, 10));

 
  if (isLoading) return <div>Loading...</div>;
  if (error) return <div>Error loading director details.</div>;
        ...
        <div className="max-w-4xl mx-auto p-6">
          <div className="bg-white p-6 rounded-lg shadow-lg mb-6 flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/3 flex justify-center mb-4 md:mb-0">
              <img
                src={`${API_BASE_URL}/${profile_picture}`} // Profile picture of the director
                alt={name}
